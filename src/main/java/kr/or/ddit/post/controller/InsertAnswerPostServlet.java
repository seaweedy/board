...
@WebServlet("/insertAnswerPost")
@MultipartConfig(maxFileSize = 1024 * 1024 * 100, maxRequestSize = 1024 * 1024 * 26)
public class InsertAnswerPostServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		int post_parent = Integer.parseInt(request.getParameter("post_seq"));
		String boardname = request.getParameter("boardname");
		String userid = request.getParameter("userid");
		String post_title = request.getParameter("post_title");
		
		request.setAttribute("post_parent", post_parent);
		request.setAttribute("boardname", boardname);
		request.setAttribute("userid", userid);
		request.setAttribute("post_title", post_title);
		
		request.getRequestDispatcher("insertAnswerPostForm.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(insertAnswerCnt == 1) { // 글을 정상적으로 등록
